 ...
 * Add your docs here.
 */
public class Pixy1Cam extends Subsystem {
...
public ArrayList<PixyBlock> read(){
  ...
  if (pixyBlocks != null && pixyBlocks.size() > 0){
    if (pixyBlocks.size() >= 2){
      ...
      setLastOffset(130); //Keeps robot going straight if only one signal is picked up
      
        }
  } else{
    setLastOffset(130); //Keeps robot going straight if nothing is picked up
    setInRange(false);
  }
  return pixyBlocks;
}
