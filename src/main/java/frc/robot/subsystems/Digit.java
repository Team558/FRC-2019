 ...
 * Add your docs here.
 */
public class Digit extends Subsystem {
  ...
  private static class Board_Task implements Runnable
	...
	private static Digit instance = new Digit();
 ...
 */
	 ...
	 * getA and getB will simply return if the button is pushed (the button being pushed returns false)
	 * getALatch and getBLatch will remain true until the data is read.
	 * getAonRisingEdge and getBOnRisingEdgeis true only when the value has gone from false to true (it will only be true when the button is released.
	 */
	public void update()
	{
	...
	public double getPotentiometer()
	{
		...
		val = 1.0 - val;
		val = (Math.round(val * 15.0)); /// Scaled to an interger between 0 - 15 (Matches total number of auto choices)
		return val;
	}
